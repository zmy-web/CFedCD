...
step1: Run DeepSets.py, the Deep Sets model is utilized to extract digest descriptors from the discretized local clients’ training data in a privacy-preserving manner. 

step2: Execute cluster.R on the server side to aggregate client-level digest vectors and partition clients into distributionally consistent clusters.The primary objective of the clustering phase is to partition the highly heterogeneous multicenter dataset into sub-populations, where the data distribution within each cluster exhibits higher consistency, thereby facilitating more reliable structure learning. 

step3: Run localCDmodel.R on client-side datasets to automatically perform local causal structure learning and output the standardized adjacency matrices, edge strengths, and network scores.

step4:Run Step5ClusterFLCD.R. Within each cluster, clients are approximated as having homogeneous distributions (representing clinically relevant patient subpopulations), enabling the server to coordinate structural learning on the original discretized data for the collaborative construction of a personalized federated directed graph.
